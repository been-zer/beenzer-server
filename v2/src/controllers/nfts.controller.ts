...
export async function newNFT(id: number, token: string, supply: number=1, creator: string, username: string, asset: string, type:string, description: string, city: string, latitude: number, longitude: number, distance: string, maxLat: string, minLat: string, maxLon: string, minLon: string): Promise<boolean> {
  ...
  console.log('ERROR: newNFT.controller failed! Token address input is ERROR or wrong!')
  return false;
}
...
export async function updateNFTOwner(token: string, newOwner: string): Promise<unknown> {
  try {
...
export async function updateNFTLikes(token: string, likes: number): Promise<boolean> {
  try {
...
export async function newNFTtransactions(token: string, owner: string, pubkey: string, type: string, currency: string, amount: number, hash: string = ''): Promise<boolean> {
  try {
    await db.query(_newNFTtransactions(token, owner, pubkey, type, currency, amount, hash));
    return true;
  } catch (error) {
    if (String(error).includes('duplicate')) {
      console.log("ERROR: Table already exists");
